...
function buildStars(count, width, height) {
  ...
  for (let i = 0; i < count; i++) {
    const h = hashText(`lumia-star-${i}`);
    const x = 30 + (h % (width - 60));
    const y = 22 + ((h >> 5) % (height - 44));
    const r = [0.8, 1, 1.2, 1.6][h % 4];
    const opacity = 0.18 + ((h >> 3) % 70) / 100;

    stars.push(`
      <circle cx="${x}" cy="${y}" r="${r}" fill="#F8FAFC" opacity="${opacity}">
        <animate
          attributeName="opacity"
          values="${opacity};${Math.min(opacity + 0.45, 1)};${opacity}"
          dur="${3 + (h % 5)}s"
          begin="${(h % 20) / 10}s"
...
function generateSvg(calendar, mode = "dark") {
  ...
  const monthsSvg = monthPositions
    .map(
      ({ month, x }) => `
      <text x="${x}" y="${gridY - 26}" class="month">${month}</text>
    `
    )
    .join("\n");
  ...
  const dayLabels = [
    ...
    .map(
      ({ label, row }) => `
      <text x="${gridX - 78}" y="${gridY + row * pitch + 16}" class="day">${label}</text>
    `
    )
    .join("\n");
...
<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <style>
    ...
    .badgeText {
      font: 800 22px Inter, Arial, sans-serif;
      fill: ${theme.accent};
      letter-spacing: 2px;
    }

    .buttonText {
      font: 800 27px Inter, Arial, sans-serif;
      fill: ${theme.text};
    }
    ...
    .totalNumber {
      font: 900 48px Inter, Arial, sans-serif;
      fill: ${theme.text};
      letter-spacing: -1px;
    }

    .ufo {
      animation: ufoMove 8s ease-in-out infinite;
    ...
    .shooting {
      animation: shooting 7.4s ease-in-out infinite;
      transform-origin: center;
    }
    ...
    @keyframes scan {
      ...
      10% { opacity: 1; }
      90% { opacity: 1; }
      to { transform: translateX(${gridW + 360}px); opacity: 0; }
    }

    @keyframes ufoMove {
      0% { transform: translateX(0px); }
      45% { transform: translateX(${Math.min(gridW - 170, 1120)}px); }
      50% { transform: translateX(${Math.min(gridW - 170, 1120)}px); }
  ...
  <g clip-path="url(#cardClip)">
    ...
    <text x="315" y="122" class="subtitle">contribuições no ano</text>
    ...
    <g clip-path="url(#gridClip)">
      ...
      <g class="ufo" transform="translate(${gridX + 40} ${gridY - 48})">
        <g class="ufoFloat">
          ...
          <circle cx="28" cy="24" r="3.5" fill="${theme.text}">
            <animate attributeName="opacity" values="0.45;1;0.45" dur="1.3s" repeatCount="indefinite"/>
          </circle>
          <circle cx="50" cy="27" r="3.5" fill="${theme.text}">
            <animate attributeName="opacity" values="1;0.45;1" dur="1.5s" repeatCount="indefinite"/>
          </circle>
          <circle cx="72" cy="24" r="3.5" fill="${theme.text}">
            <animate attributeName="opacity" values="0.45;1;0.45" dur="1.2s" repeatCount="indefinite"/>
          </circle>

          <polygon
            class="beamPulse"
            points="29,35 71,35 122,250 -22,250"
